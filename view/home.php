...
<body>
    ...
    <div class="pagination"
        style="text-align: center; margin: 30px 0; display: flex; justify-content: center; align-items: center; gap: 10px;">
        <?php if ($page > 1): ?>
            <a href="?page=<?= $page - 1 ?>"
                style="padding: 10px 20px; border: 1px solid #d1d5db; border-radius: 6px; text-decoration: none; color: #111827; font-weight: 500;">
                PREV</a>
        <?php endif; ?>
        ...
        for ($i = 1; $i <= $totalPages; $i++) {
            if ($i == $page) {
                echo '<span style="padding: 10px 15px; background-color: #2563eb; color: white; border-radius: 6px; font-weight: bold;">' . $i . '</span>';
            } else {
                echo '<a href="?page=' . $i . '" style="padding: 10px 15px; text-decoration: none; color: #111827; border-radius: 6px;">' . $i . '</a>';
            }
        }
        ?>
        <?php if ($page < $totalPages): ?>
            <a href="?page=<?= $page + 1 ?>"
                style="padding: 10px 20px; border: 1px solid #d1d5db; border-radius: 6px; text-decoration: none; color: #111827; font-weight: 500;">NEXT
            </a>
        <?php endif; ?>
    </div>
